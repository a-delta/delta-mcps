...
A reusable deck engine in the **Tesco visual identity**, delivered as a **single
self-contained HTML file**. Brand — colours, Tesco Modern font, logo — is baked in;
the slides themselves are generic examples you replace with your own content.
...
- **Theme:** `T` toggles light / dark (both official Tesco themes). **Fullscreen:** `F`.
  **Help:** `?`.
...
assets/
  fonts/            Tesco Modern woff2 (base64-embedded at build)
  brand/            Tesco logo SVG (inlined at build)
