...
This is a **presentation template**: a deck engine in the Prioclen Consulting (PCL)
visual identity that builds to **one self-contained HTML file**. Your job when asked to
"make a deck" or "add slides" is to author small HTML slide fragments and wire them into
a manifest — not to reinvent the engine.
...
2. **Keep the brand.** Colours, fonts and logo are Prioclen Consulting's and are fixed.
   Reference **tokens** (`var(--pcl-green)`, `var(--surface)`, …) — never hard-code a
   hex, font or colour in a slide. All brand values live in `src/styles/tokens.css`;
...
- Confirm: fonts load (Aptos), titles are PCL green, the logo renders, fragments
...
- ❌ Hard-coding `#078045` (or any hex/font) in a slide — use a token.
