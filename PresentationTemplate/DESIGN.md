...
The deck ships in **Prioclen Consulting's visual identity**. All brand values live in
[`src/styles/tokens.css`](src/styles/tokens.css) — change them *there* and everything
downstream updates. Full palette + typography notes: [`assets/reference/prioclen-brand.md`](assets/reference/prioclen-brand.md).

### Colour

| Token | Value | Use |
|-------|-------|-----|
| `--pcl-green` | `#078045` | **Primary.** Titles, chrome, primary accent. |
| `--pcl-green-deep` | `#016837` | **Secondary.** Gradients, dark-theme chrome. |
| `--pcl-green-light` | `#35c98a` | Primary lightened — titles/labels on dark. |
| `--pcl-orange` | `#f2622e` | The "full-stop" accent, emphasis, alert states. |
| `--pcl-white` | `#ffffff` | **Secondary.** Surfaces; text on brand green. |
| `--pcl-light-grey` | `#f4f6f5` | Page background (light theme). |
| `--success` / `--warning` / `--info` | green / amber / blue | Status accents. |
| `--sev-critical…low` | purple→yellow | A severity scale for badges/bars. |
| `--accent-a` / `--accent-b` | `#0aa35a` / `#D689FF` | Secondary/tertiary card stripes. |
...
- **Aptos** (self-hosted woff2, Latin-subset, base64-embedded at build). Weights
  300/400/500–600/700–800/900 plus Regular + Bold italics — `--font-sans`.
- **Headers** use `--font-display` (**Aptos Display**, falling back to Aptos — see
  [the brand notes](assets/reference/prioclen-brand.md#typography)); body copy uses Aptos Regular.
- `h1`/`h2` render in PCL green (a lighter green on dark for legibility); `h3` in strong text.
- Sizes are fluid (`clamp(...)`) so slides scale from laptop to projector.

### Two house rules

- **The orange full-stop.** End a title with `<span class="dot">.</span>` — an orange
  period. Reserved for that only; **never** colour a `?` or other punctuation orange.
- **The kicker eyebrow.** A small uppercase label above the title, prefixed by a round
  orange dot (`<div class="kicker">…</div>`). Use it to name the section or slide type.
...
- **light** (default) — white surfaces on PCL light-grey.
- **dark** — deep green-black with a subtle branded radial gradient.
...
3. **Font tokens** — `__FONT_LIGHT__` … `__FONT_BOLD_ITALIC__` are replaced with base64
   woff2. `__FONT_FACE_DISPLAY__` becomes the Aptos Display `@font-face` rules if those
   files are present in `assets/fonts/`, and nothing at all if they are not.
4. **Logo tokens** — `__LOGO_SVG__` is replaced with the inlined `pcl-logo.svg`
   (the horizontal lockup) and `__LOGO_MARK_SVG__` with `pcl-mark.svg` (the mark alone,
   for the 26px chrome slots). A `viewBox` is injected if missing, so `height +
   width:auto` scales cleanly.
